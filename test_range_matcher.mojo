...
from regex.simd_matchers import RangeBasedMatcher, create_digit_matcher, create_alpha_matcher, create_alnum_matcher
...
fn test_alpha_range_matcher() raises:
    ...
    for i in range(min(len(test_data), 16)):
        var ch = test_data[i]
        var is_match = matches[i]
        var expected = (ord(ch) >= ord('a') and ord(ch) <= ord('z')) or (ord(ch) >= ord('A') and ord(ch) <= ord('Z'))
        print("  '", ch, "' -> matched:", is_match, ", expected:", expected)
        if is_match != expected:
...
fn benchmark_range_vs_nibble() raises:
    ...
    print("  Throughput:", Float64(len(test_data)) / Float64(range_time) * 1e9 / 1e6, "MB/s")
    ...
    print("  Throughput:", Float64(len(test_data)) / Float64(simd_time) * 1e9 / 1e6, "MB/s")
