...
## High: `Optional[String]` in _extract_character_class_info

**File:** `src/regex/dfa.mojo` lines 2292-2362

Changed from `Optional[StringSlice[ImmutAnyOrigin]]` (zero-copy) to
`Optional[String]` (allocates) during the 0.26.2 migration. This causes a string
copy for every `[0-9]+` or `\d+` pattern compilation.

Benchmark impact: `range_digits` 2.7x slower than 0.25.7, `predefined_digits`
2.0x slower.

Fix: Find a way to use `StringSlice` with a compatible origin, or use a `Span[Byte]`
