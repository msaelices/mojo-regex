...
struct PatternAnalyzer:
    ...
    fn _analyze_alternation(
        ...
        for i in range(ast.children_len()):
    ...
    fn _analyze_group(self, ast: ASTNode, depth: Int) -> PatternComplexity:
        ...
        if (
            ...
            if (
                all_literal and ast.children_len() <= 20
            ):  # Allow longer literal strings
                return PatternComplexity(PatternComplexity.SIMPLE)
            elif ast.children_len() <= 3:
                return PatternComplexity(PatternComplexity.SIMPLE)
            else:
                return PatternComplexity(PatternComplexity.MEDIUM)
        else:
    ...
    fn _is_multi_char_class_sequence(self, ast: ASTNode) -> Bool:
        ...
        for i in range(ast.children_len()):
            ref element = ast.children_ptr[][i]
            if not (
                element.type == RANGE
...
fn _is_literal_sequence(ast: ASTNode) -> Bool:
    ...
    if ast.type == ELEMENT:
        ...
        for i in range(ast.children_len()):
            if not _is_literal_sequence(ast.children_ptr[][i]):
                return False
        return True
    else:
...
fn get_literal_string(ast: ASTNode) -> String:
    ...
    """
    if ast.type == RE and ast.children_len() > 0:
        return _extract_literal_chars(ast.children_ptr[][0])
    else:
        return ""
...
fn _extract_literal_chars(ast: ASTNode) -> String:
    ...
    if ast.type == ELEMENT:
        ...
        var result = String("")
        for i in range(ast.children_len()):
            result += _extract_literal_chars(ast.children_ptr[][i])
        return result
    elif ast.type == START or ast.type == END:
