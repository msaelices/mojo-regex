...
struct LiteralInfo(Copyable, Movable):
    ...
    fn has_prefilter_candidates(self) -> Bool:
        ...
        return (len(self.required_literals) > 0 or 
                len(self.literal_prefixes) > 0 or
                len(self.literal_suffixes) > 0)
...
struct LiteralExtractor:
    ...
    fn _extract_from_node(self, ast: ASTNode[MutableAnyOrigin], mut info: LiteralInfo):
    ...
    fn _extract_literal_sequence(self, ast: ASTNode[MutableAnyOrigin]) -> String:
        ...
        if ast.type == ELEMENT:
            ...
            var result = String()
            for i in range(ast.get_children_len()):
                var child_literal = self._extract_literal_sequence(ast.get_child(i))
    ...
    fn _extract_alternation_literals(self, ast: ASTNode[MutableAnyOrigin], mut info: LiteralInfo):
    ...
    fn _collect_alternation_branches(self, ast: ASTNode[MutableAnyOrigin], mut branches: List[String]):
...
trait PrefilterMatcher:
    ...
    fn find_first_candidate(self, text: String, start: Int = 0) -> Optional[Int]:
