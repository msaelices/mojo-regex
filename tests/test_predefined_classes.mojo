...
from regex.matcher import match, findall
from regex.nfa import match as nfa_match
...
def test_digit_unicode_edge_cases():
    ...
    var result1 = match("\\d", "0")
    assert_true(result1)

    var result2 = match("\\d", "9")
    assert_true(result2)

    # Non-ASCII should not match
    var result3 = match("\\d", "a")
...
def test_word_digits_and_underscore():
    ...
    for i in range(10):
        var digit_char = String(i)
        var result = match("\\w", digit_char)
        assert_true(result)
        var matched = result.value()
        assert_equal(matched.get_match_text(), digit_char)

    # Test underscore
    var result = match("\\w", "_")
    assert_true(result)
    var matched = result.value()
...
def test_word_quantifier_basic():
    ...
    # Test + quantifier
    var result1 = match("\\w+", "hello123")
    assert_true(result1)
    var matched1 = result1.value()
    assert_equal(matched1.get_match_text(), "hello123")

    # Test * quantifier
    var result2 = match("\\w*", "hello_world")
    assert_true(result2)
    var matched2 = result2.value()
...
def test_anchored_patterns():
    ...
    var result2 = match("^\\d+$", "abc123")
    assert_false(result2)  # Doesn't start with digit

    var result3 = match("^\\w+$", "hello_123")
    assert_true(result3)
    var matched3 = result3.value()
...
def test_greedy_vs_minimal_matching():
    ...
    var result1 = match("\\d+", "123abc")
    assert_true(result1)
    var matched1 = result1.value()
    assert_equal(matched1.get_match_text(), "123")

    # \\w+ should be greedy and match all consecutive word characters
    var result2 = match("\\w+", "hello123_world@")
    assert_true(result2)
    var matched2 = result2.value()
...
def test_single_character_strings():
    ...
    var result4 = match("\\w", "Z")
    assert_true(result4)

    var result5 = match("\\w", "_")
    assert_true(result5)
...
def test_very_long_strings():
    ...
    var result1 = match("\\d+", long_digits)
    assert_true(result1)
    var matched1 = result1.value()
    ...
    var result2 = match("\\w+", long_words)
    assert_true(result2)
    var matched2 = result2.value()
...
def test_multiple_consecutive_patterns():
    ...
    var result1 = match("\\d\\d\\d", "123")
    assert_true(result1)
    var matched1 = result1.value()
    assert_equal(matched1.get_match_text(), "123")

    var result2 = match("\\w\\w\\w", "abc")
    assert_true(result2)
    var matched2 = result2.value()
    assert_equal(matched2.get_match_text(), "abc")

    # Mixed patterns
    var result3 = match("\\w\\d\\w", "a5z")
    assert_true(result3)
    var matched3 = result3.value()
...
def test_nested_quantifiers():
    ...
    var result1 = match("(\\w\\d)+", "a1b2c3")
    assert_true(result1)
    var matched1 = result1.value()
    assert_equal(matched1.get_match_text(), "a1b2c3")

    var result2 = match("(\\w\\d)+", "ab12")
...
def test_performance_stress():
    ...
    var result1 = match("\\d+", many_digits)
    assert_true(result1)
    var matched1 = result1.value()
    ...
    var result2 = match("\\w+", many_words)
    assert_true(result2)
    var matched2 = result2.value()
...
def test_nfa_digit_direct():
    ...
    var result1 = nfa_match("\\d", "5")
    assert_true(result1)
    var matched1 = result1.value()
    assert_equal(matched1.get_match_text(), "5")

    var result2 = nfa_match("\\d", "a")
    assert_false(result2)

    var result3 = nfa_match("\\d*", "@#$")
    assert_true(result3)
    var matched3 = result3.value()
    assert_equal(matched3.get_match_text(), "")
...
def test_nfa_word_direct():
    ...
    var result1 = nfa_match("\\w", "a")
    assert_true(result1)
    var matched1 = result1.value()
    assert_equal(matched1.get_match_text(), "a")

    var result2 = nfa_match("\\w", "@")
    assert_false(result2)

    var result3 = nfa_match("\\w*", "@#$")
    assert_true(result3)
    var matched3 = result3.value()
    assert_equal(matched3.get_match_text(), "")
...
def test_digit_word_regression():
    ...
    var result1 = match("\\d", "5")
    assert_true(result1)

    var result2 = match("\\w", "a")
    assert_true(result2)

    # Zero-match quantifier cases
    var result3 = match("\\d*", "abc")
    assert_true(result3)
    var matched3 = result3.value()
    assert_equal(matched3.get_match_text(), "")
...
def test_complex_real_world_patterns():
    ...
    var result1 = match("\\w+", "variable123")
    assert_true(result1)
    var matched1 = result1.value()
    assert_equal(matched1.get_match_text(), "variable123")

    # Number at start of string
    var result2 = match("\\d+", "123abc")
    assert_true(result2)
    var matched2 = result2.value()
    assert_equal(matched2.get_match_text(), "123")

    # Word characters at start
    var result3 = match("\\w+", "hello_123@world")
    assert_true(result3)
    var matched3 = result3.value()
    assert_equal(matched3.get_match_text(), "hello_123")
...
def test_digit_boundary_values():
    ...
    var result1 = match("\\d", chr(ord("0") - 1))  # '/' character
    assert_false(result1)

    var result2 = match("\\d", chr(ord("9") + 1))  # ':' character
    assert_false(result2)

    # Test actual boundaries
    var result3 = match("\\d", "0")
    assert_true(result3)

    var result4 = match("\\d", "9")
    assert_true(result4)
...
def test_word_boundary_values():
    ...
    var result1 = match("\\w", chr(ord("a") - 1))  # '`' character
    assert_false(result1)

    var result2 = match("\\w", chr(ord("z") + 1))  # '{' character
    assert_false(result2)

    # Test ASCII values around uppercase letters
    var result3 = match("\\w", chr(ord("A") - 1))  # '@' character
    assert_false(result3)

    var result4 = match("\\w", chr(ord("Z") + 1))  # '[' character
    assert_false(result4)

    # Test actual boundaries
    var result5 = match("\\w", "a")
    assert_true(result5)

    var result6 = match("\\w", "z")
    assert_true(result6)

    var result7 = match("\\w", "A")
    assert_true(result7)

    var result8 = match("\\w", "Z")
    assert_true(result8)
